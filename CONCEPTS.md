...
The three parallel identity sets of the source model: provider ids (key-backed API integrations), engine ids (keyless navigable search engines), and source ids (the union of both plus user-defined Site Engines and Custom Engines). The sets deliberately stay separate — provider-backed execution and engine navigation never share an id — and the only cross-cutting merge is SourceId, the composition point that the UI, storage, and the SERP Switch Bar all speak.
