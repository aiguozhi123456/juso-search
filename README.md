...
独立搜索页提供搜索来源选择和切换；在 Google、Bing、Baidu、抖音、小红书的受支持结果页上，SERP 切换栏可将当前查询直接切到其他搜索引擎，或跳转至 Juso 的 AI 搜索页。
...
1. 按“安装与更新”安装并启用扩展。
2. 打开 Juso 搜索页并选择搜索来源。Google、Bing、Baidu、抖音、小红书无需配置；只有使用 AI 搜索服务时，才需要在扩展设置中配置对应服务的密钥。

完成后，你可以在一个入口搜索、切换 Google、Bing、Baidu、抖音、小红书和已配置的 AI 搜索服务。
