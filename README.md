...
1. 按上面的步骤安装并启用扩展。使用 `engine-search` 检索传统搜索引擎无需配置 AI 搜索服务；只有通过 `search --provider` 调用 AI 搜索 API 时，才需要先配置对应服务。
2. 将 `skills/juso-search/` 安装或复制到你的智能体技能目录，例如 `.agents/skills/juso-search/`。
3. 打开 `chrome://extensions`（Edge 使用 `edge://extensions`），开启“开发者模式”，在 Juso 扩展卡片上复制 32 位扩展 ID。
4. 将该 ID 设置为 `JUSO_EXTENSION_ID`，或在命令中传入 `--extension-id`：

```powershell
$env:JUSO_EXTENSION_ID = "你的扩展 ID"
```
...
也可以不设置环境变量，直接使用 `python scripts/juso_search.py --extension-id YOUR_EXTENSION_ID list-providers`。如果以不同目录加载另一份扩展副本，扩展 ID 可能变化，请重新复制。
...
`search` 必须提供 `--provider`，不会悄悄跟随扩展当前服务。`engine-search` 仅提取普通结果链接，不承诺 AI 摘要、知识面板或其他页面内容；取得 URL 后，页面抓取应由智能体宿主自己的 `web_fetch` 等能力完成。遇到浏览器无法启动、扩展未连接、超时、验证页、同意页、页面布局不支持或无结果时，命令会失败；请检查扩展 ID、浏览器配置和服务配置，不要通过暴露密钥来重试。
