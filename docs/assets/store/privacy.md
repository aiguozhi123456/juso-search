...
## 4. 权限理由 — 主机权限(限 1000 字符;实测 988)

> (1) Provider APIs (api.tavily.com, api.exa.ai, api.stepfun.com): the worker fetches the user's AI providers, sending only the query, the user's key, and minimal params; the Stepfun MCP client calls only 'web_search'. No cookies, history, or page content is collected.
>
