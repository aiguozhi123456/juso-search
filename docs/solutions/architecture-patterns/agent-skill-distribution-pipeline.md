...
- `VARIANTS` (`scripts/gen_skills.py:24-33`) maps `prod` → `skills/juso-search/` and `dev` → `skills/juso-search-dev/`, each carrying only its extension id and target directory.
- `render(variant_key)` (`:104`) reads each template file, and for `dev` only, applies `DEV_PATCH_SKILL_MD` / `DEV_PATCH_PY` — the encoded prod→dev prose diff as `(find, replace)` pairs. The final pass substitutes `__JUSO_EXTENSION_ID__` everywhere, including any id the patches themselves injected.
- `_apply_patch` (`:89`) enforces a **loud-match contract**: every `find` string must occur *exactly once* in the template. A count of 0 or 2 raises `SystemExit` with the offending snippet. A partial or ambiguous transform can never silently ship.

This is why the template carries only the id placeholder and no name/description/warning placeholders: those differences are *transforms on prod prose*, expressed as find/replace pairs, not slots to fill. The pairs are the authoritative record of "what makes dev different from prod".
...
1. `const extId = browser.runtime.id` (`:45`), validated against `EXTENSION_ID_RE = /^[a-p]{32}$/`. A mismatch throws with a message that flags a dev/custom build as the expected cause rather than a generic failure.
2. `fetch(browser.runtime.getURL('agent-skill/SKILL.md'))` and `.../scripts/juso_search.py` read the bundled template.
3. `stamp(text, extId)` (`:21`) splits on `__JUSO_EXTENSION_ID__` and joins with the runtime id. A post-stamp check (`:63`) throws if any placeholder remains — a guard against template drift sneaking past the build.
4. `createStoreZip` (`lib/zip.ts`) packages the two entries under a **uniform** `juso-search/` top-level folder, with auto-inserted directory entries (`juso-search/`, `juso-search/scripts/`).
...
### Two build-time seams, one purpose each

The pipeline uses two distinct seams that are easy to conflate:
...
Keeping these seams separate is the reason a custom dev build (unknown id, but `mode === 'development'` so `__SKILL_VARIANT__ === 'dev'`) produces a correctly-named `juso-search-dev-<version>.zip` whose content is stamped with the *user's own* runtime id.
...
```text
 public/agent-skill/  (single source: prod content + __JUSO_EXTENSION_ID__ placeholder)
        │
...
- **Do not conflate `__JUSO_EXTENSION_ID__` (runtime) with `__SKILL_VARIANT__` (build-time).** The first is a content placeholder resolved per-download from `browser.runtime.id`; the second is a compile-time constant used only for the zip filename token. Routing variant content through `__SKILL_VARIANT__` would reintroduce a parallel content path and defeat the single-template property.
