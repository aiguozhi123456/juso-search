...
> **Floating-popup escape (see [wxt-shadow-root-inline-style-clobber](../ui-bugs/wxt-shadow-root-inline-style-clobber.md)):** this host-restore discipline applies when the host is itself the visible in-flow element (the SERP bar). A *floating* popup host (selection-search) has a simpler alternative: keep the host zero-sized/static and position an inner wrapper inside the shadow root — the `all: initial` reset only touches the host element, not its descendants. It is also a second consumer of the §4d-4e hover-intent/outside-dismiss state machine (150ms, plain hover vs the bar's 120ms click-to-pin) and deliberately uses `position:absolute` + `scrollX/scrollY` page coordinates so the popup scrolls with the page — contrasting with the bar's `position:fixed` viewport-coordinate overlays.
