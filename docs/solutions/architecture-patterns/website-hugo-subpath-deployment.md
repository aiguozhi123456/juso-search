...
- **Partials for reuse**: `partials/header.html`, `partials/footer.html`, and section partials (`capability-matrix.html`, `cli.html`, `icon-wall.html`, etc.) are the reuse layer. Six surfaces share one skeleton via `layouts/_default/baseof.html`: `layouts/index.html` (neutral overview), `layouts/human/single.html` / `layouts/agents/single.html` (the two faces), and the docs guides added 2026-08-12 — `layouts/docs/list.html` (`/docs/`), `layouts/human-docs/single.html` (`/human/docs/`), `layouts/agents-docs/single.html` (`/agents/docs/`).
...
- **Mechanism**: GitHub Actions (`.github/workflows/website-pages.yml`) — `peaceiris/actions-hugo` builds, `actions/deploy-pages` deploys. Triggered on pushes to `main` that touch `website/**`, the drift-lock source trees (`entrypoints/shared/tokens.css`, `public/icons/**`, `docs/assets/screens/**`), or the workflow file (plus `workflow_dispatch`).
...
- The site's surfaces (`/`, `/human/`, `/agents/`, `/docs/`, `/human/docs/`, `/agents/docs/`) share one skeleton but swap content via `layouts/index.html` (neutral overview) vs `layouts/human/single.html` (human face) vs `layouts/agents/single.html` (agent face), all backed by `layouts/_default/baseof.html`.
