...
This project ships two independent product lines from one repo: a Chrome MV3 extension (WXT + React, versioned `1.4.0`) and a Python MCP pip package `juso-search` (versioned `0.1.1`). A version audit found that the same version line was hardcoded in multiple places, and one of those copies had silently drifted:
...
- `mcp-server/pyproject.toml` hardcoded `version = "0.1.1"` separately from `juso_search/__init__.py`'s `__version__` — same structural duplication.
...
[project]
name = "juso-search"
version = "0.1.1"   # second hardcoded source — drifts from __init__.py
...
The extension (`1.4.0`, Chrome Web Store) and the MCP package (`0.1.1`, PyPI) release on different schedules for different artifacts. Do not couple them into one shared version number — that creates artificial release coordination overhead. Two independent single-source lines is correct; the principle is "one source *per line*," not "one source for the whole repo."
...
__init__.py         → __version__ = "0.1.1"  ← single source (MCP line)
...
- `npm test` → 1223 tests pass, including the new `version-drift` gate.
- `npm run test:mcp` → 38 tests pass, including the dualera handshake that reads `__version__`.
- `npm run test:python` → 52 tests pass.
