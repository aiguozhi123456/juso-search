...
### 2. Page-scoped hero enlargement — body class by page type

The homepage hero is deliberately larger than the agent page hero: wider max-width (1240px vs 1160px), a `0.75fr 1fr` grid (visual-dominant), and stretched visual column. This sizing is scoped via a **body class named after the page type**, not the visual treatment:
...
```css
/* style.css — scoped under page-type class */
.is-home .hero__inner { max-width: 1240px; grid-template-columns: 0.75fr 1fr; }
.is-home .hero__visual { max-width: none; justify-self: stretch; }
```

**Why `is-home` not `hero-enlarged`:** the class name should describe *which page* gets the treatment, not *what the treatment is*. A future developer adding a third page type reads `is-home` and knows it's a page-type flag; `hero-enlarged` describes a CSS consequence that requires reading the stylesheet to understand. (Renamed from `hero-enlarged` → `is-home` in the maintainability refactor.)

The agent page (`agents/single.html`) has no body class and inherits the default hero proportions — its CLI demo block is compact and doesn't need the enlarged visual column.
...
**Page-type body classes scale better than per-page stylesheets.** The homepage and agent page share `baseof.html`. A body class is the cheapest way to diverge on a single layout dimension (hero sizing) without forking the base template or adding conditional stylesheet loading.
...
- **Scoping a new page-type visual treatment:** add a body class (`is-home`, `is-agent`, etc.) and scope CSS under it. Do not use presentation-descriptive names (`hero-enlarged`, `wide-layout`).
...
|---|---|---|---|
| Homepage (`/`) | `is-home` | `0.75fr 1fr`, max-width 1240px | stretched (full column width) |
| Agent (`/agents`) | (none) | default (`1fr`), max-width 1160px | `max-width: 520px` |
