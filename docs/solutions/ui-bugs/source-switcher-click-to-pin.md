...
- [wxt-shadow-root-inline-style-clobber](../ui-bugs/wxt-shadow-root-inline-style-clobber.md) — 划词搜索弹窗复用同一 hover-intent + click-to-pin 模式（SelectionSearchPopup，150ms 延迟），与本文档共享 scheduleClose/cancelClose + pinnedRef 守卫 + 卸载清理结构；适配到 shadow DOM 嵌套浮层时的五个分歧点见 [selection-popup-click-to-pin-shadow-dom-adaptation](../design-patterns/selection-popup-click-to-pin-shadow-dom-adaptation.md)。共享浮层状态机锚点在 [serp-bar-bottom-position-and-scroll-hide](../architecture-patterns/serp-bar-bottom-position-and-scroll-hide.md) §4d-4e。
