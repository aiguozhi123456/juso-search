...
- **Any release that adds a manifest `permissions` entry** (e.g., `contextMenus`) — the CWS questionnaire's permission-reasons section must cover each declared permission, so a permission addition is its own audit trigger, not only a side effect of a feature change
...
- [Chrome & Firefox Extension Release Process](./chrome-extension-release-process.md) — the full release workflow (version bump, Chrome dev/prod + Firefox ZIP build, tagging, GitHub Release, CWS upload)
