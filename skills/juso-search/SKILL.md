...
- 扩展 ID 已内置默认值（Chrome Web Store 发布版），无需手动配置。仅在自行签名打包（扩展 ID 与默认不一致）时才需设置 `JUSO_EXTENSION_ID` 或传 `--extension-id` 覆盖。若使用开发版（`npm run build:dev` 构建），请改用 [juso-search-dev](https://github.com/aiguozhi123456/juso-search/tree/main/skills/juso-search-dev) 技能。
