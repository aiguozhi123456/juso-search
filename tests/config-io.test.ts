...
describe('buildExportPayload', () => {
  ...
  it('exports a normalized complete source order', async () => {
    ...
    await expect(buildExportPayload()).resolves.toMatchObject({
      sourceOrder: ['bing', 'exa', 'tavily', 'stepfun', 'stepfun-plan', 'google', 'baidu', 'douyin', 'xiaohongshu', 'bilibili'],
    });
  });
...
describe('parseImportPayload', () => {
  ...
  it('normalizes a valid partial sourceOrder by appending missing sources', () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.value.sourceOrder).toEqual(['bing', 'tavily', 'exa', 'stepfun', 'stepfun-plan', 'google', 'baidu', 'douyin', 'xiaohongshu', 'bilibili']);
  });
...
describe('mergeImport', () => {
  ...
  it('writes sourceOrder only when applying preferences', async () => {
    const payload = validPayload({ sourceOrder: ['bing', 'tavily', 'exa', 'stepfun', 'stepfun-plan', 'google', 'baidu', 'douyin', 'xiaohongshu', 'bilibili'] });
  ...
  it('keeps a later source order move after an earlier import completes', async () => {
    ...
    const importedOrder: SourceId[] = ['bing', 'exa', 'google', 'tavily', 'stepfun', 'stepfun-plan', 'baidu', 'douyin', 'xiaohongshu', 'bilibili'];
    const movedOrder: SourceId[] = ['exa', 'bing', 'google', 'tavily', 'stepfun', 'stepfun-plan', 'baidu', 'douyin', 'xiaohongshu', 'bilibili'];
...
describe('previewImport (dry-run)', () => {
  ...
  it('reports a preference diff when only the normalized source order differs', async () => {
    ...
    expect(preview.prefDiffs).toEqual([{
      key: 'sourceOrder',
      from: 'bing > tavily > exa > stepfun > stepfun-plan > google > baidu > douyin > xiaohongshu > bilibili',
