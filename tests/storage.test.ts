...
describe('storage: source order', () => {
  it('round-trips a normalized complete order', async () => {
    ...
    expect(await getSourceOrder()).toEqual(['bing', 'exa', 'google', 'tavily', 'baidu', 'stepfun', 'stepfun-plan', 'jina', 'doubao', 'doubao-global', 'douyin', 'xiaohongshu', 'bilibili']);
  });

  it('normalizes invalid stored values', async () => {
    await browser.storage.local.set({ sourceOrder: ['bing', 'ghost', 'bing'] });
    expect(await getSourceOrder()).toEqual(['bing', 'tavily', 'exa', 'stepfun', 'stepfun-plan', 'jina', 'doubao', 'doubao-global', 'google', 'baidu', 'douyin', 'xiaohongshu', 'bilibili']);
  });
});
